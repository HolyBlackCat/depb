...
$(call Library,zlib,zlib-1.2.11.tar.gz,TarGzArchive,Custom,make -f win32/Makefile.gcc install --no-print-directory "CC=$(CC)" "CXX=$(CXX)" "INCLUDE_PATH=$(prefix)/include" "LIBRARY_PATH=$(prefix)/lib" "BINARY_PATH=$(prefix)/bin")
else ifneq ($(mode),)
$(error Not sure how to build zlib for this mode. Please fix `config.mk`.)
endif

$(call Library,freetype,freetype-2.10.1.tar.gz,TarGzArchive,ConfigureMake,--with-zlib --without-bzip2 --without-png --without-harfbuzz)
